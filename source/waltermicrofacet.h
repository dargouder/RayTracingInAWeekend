...
class WalterMicrofacet : public Material {
 public:
  ...
  float FresnelTerm(const float &dot_i_m, const float &refr_index_ratio) const
  {
    ...
    float F = 0.5f * ( gmc*gmc / gpc*gpc ) * ( 1.0f + (num*num) / (den*den) );
                 ...
                 float& pdf) 
  {
    ...
    Vec3 reflection_half_vec = copysign(1, Vec3::dot(incident_ray, macro_normal)) * (incident_ray + reflected_ray);
  ...
  void tr_calc(const Vec3 &incident_ray, const Vec3 &micro_normal,
               ...
               const float dot_i_n,
               Vec3 &transmitted_ray, float &brdf,
               float &pdf) const 
  {
    float refractive_index_ratio = m_eta_in / m_eta_out;
	float sign_i_m = dot_i_m < 0 ? -1 : 1;
	float sign_i_n = dot_i_n < 0 ? -1 : 1;
    float sqrt_res = sqrtf(1 + refractive_index_ratio * (sign_i_m*sign_i_m - 1));
    transmitted_ray = micro_normal * (refractive_index_ratio * dot_i_m -  sign_i_m * sqrt_res) - (incident_ray * refractive_index_ratio);
