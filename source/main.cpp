...
Vec3 colourRecursive(const Ray &ray, const Hitable &world, int depth) {
    ...
    if (world.hit(ray, 0.001, FLT_MAX, rec)) {
        ...
        if (depth < 10 && rec.mat_ptr->fr(ray, rec, attenuation, scattered, pdf)) {
            return emitted +
                   (attenuation * rec.mat_ptr->ScatteredPdf(ray, rec, scattered) *
...
void simpleLight(HitableList &list) {
    list.list.push_back(std::make_unique<Sphere>(Vec3(0.0f, -1000.0f, 0.0f), 1000.0f,
                                                 std::make_unique<Lambertian>(Vec3(0.8f, 0.1f, 0.1f))));
    list.list.push_back(std::make_unique<Sphere>(Vec3(0.0f, 2.0f, 0.0f), 2.0f,
                                                 std::make_unique<Lambertian>(Vec3(0.2f, 0.5f, 0.5f))));
    list.list.push_back(std::make_unique<Sphere>(Vec3(0.0f, 7.0f, 0.0f), 2.0f,
                                                 std::make_unique<DiffuseLight>(Vec3(4.0f, 4.0f, 4.0f))));
    list.list.push_back(std::make_unique<XYRect>(3.0f, 5.0f, 1.0f, 3.0f, -2.0f,
                                                 std::make_unique<DiffuseLight>(Vec3(4.0f, 4.0f, 4.0f))));
}
